...
Recording modes:
  record_and_transcribe()        — PTT (terminal UI), waits for Enter to stop
  record_and_transcribe_toggle() — PTT (Tk UI), stops via an external
                                    threading.Event instead of input()
...
def record_until_enter() -> Path:
    """PTT (terminal UI) — record until user presses Enter."""
...
def record_until_stopped(stop_event) -> Path:
    """PTT (Tk UI) — record until stop_event is set externally,
    instead of blocking on input(). input() waits on the terminal's
    stdin, which has no meaning for a GUI button click, so this is
    what jarvis-tk.py's toggle-style PTT button uses instead. Pass a
    threading.Event(); call .set() on it from wherever the "stop"
    action happens.
    """
    print("\033[1;32m●  Recording... (stop signaled externally)\033[0m")
    frames = []
    mic_level_buffer.clear()
...
def record_and_transcribe() -> str:
    """PTT (terminal UI) — record until Enter, transcribe, clean up."""
...
def record_and_transcribe_toggle(stop_event) -> str:
    """PTT (Tk UI) — record until stop_event is set, transcribe, clean up."""
    audio_path = record_until_stopped(stop_event)
    try:
        return transcribe(audio_path)
    finally:
        audio_path.unlink(missing_ok=True)
